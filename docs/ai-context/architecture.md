...
### ADR-012: 미리보기 리사이즈의 애니메이션 보존 범위 정정

- 날짜: 2026-06-08
- 상태: Accepted
- Refines: ADR-008 (리사이즈 전 base64 임베드 구조는 유지, 멀티프레임 판별 술어만 정정)
- 맥락: ADR-008은 `n_frames > 1` 이미지를 모두 애니메이션으로 보고 원본 passthrough하도록 기록했다. 그러나 iPhone MPO는 여러 프레임을 담는 정지영상인데도 Pillow에서 `format == "MPO"`, `n_frames == 2`, `is_animated == True`로 보인다. 이 때문에 4032×3024 MPO 사진들이 다운스케일되지 않아 미리보기 HTML이 과도하게 커졌다.
- 결정: `_resize_image_bytes`는 진짜 애니메이션 짤방만 보존한다. 구체적으로 `fmt in {"GIF", "WEBP"}`이고 `is_animated`인 경우만 원본 프레임을 보존한다. 그 외 `n_frames > 1` 이미지는 멀티프레임 정지영상으로 보고 `seek(0)` 후 기존 EXIF 보정·thumbnail·재인코딩 경로를 탄다.
- 이유: GIF/WebP 움짤은 시간 흐름 자체가 콘텐츠라 평탄화하면 안 되지만, MPO 같은 멀티프레임 정지영상은 대표 프레임을 축소해야 preview HTML 용량 절감이라는 ADR-008의 본래 목적을 달성한다. `n_frames`/`is_animated`는 둘을 구분하는 도메인 신호가 아니며, 이 프로젝트의 애니메이션 자산 범위는 GIF/WebP다.
- 대안: `n_frames > 1` 전부 passthrough 유지(용량 버그 유지); 모든 멀티프레임을 강제 평탄화(GIF/WebP 움짤 파괴); 크기 가드 추가(근본 원인 해결이 아니라 후속 운영 정책).
- 트레이드오프: APNG/AVIF 애니메이션은 이번 보존 화이트리스트에 포함하지 않는다. 현재 짤방 수집·미리보기 범위가 GIF/WebP 중심이므로 단순한 판별 술어를 유지한다.

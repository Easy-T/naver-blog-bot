...
| 클립보드 복사 | `preview` 실행 시 붙여넣기용 텍스트를 클립보드에 복사. 네이버 SmartEditor에 붙여넣기 위해 사용. |
| 붙여넣기용 텍스트 | 초안 본문의 사진·짤방·이모티콘 마커를 사람이 읽을 삽입 단서(예: 〔📷 사진 삽입: 파일명〕)로 바꾼, SmartEditor에 그대로 붙여넣는 본문. `preview`가 클립보드와 텍스트 파일로 제공. 파일 경로는 노출하지 않고 파일명만 표시. |
